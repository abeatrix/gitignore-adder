...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('gitignore-adder.createFile', () => {
	...
	let disposable2 = vscode.commands.registerCommand('gitignore-adder.addFile', (uri:vscode.Uri) => {
		// The code you place here will be executed every time your command is executed
		if(vscode.workspace.workspaceFolders){
			const rootDir = vscode.workspace.workspaceFolders[0].uri.fsPath;
			const filePath = vscode.Uri.file(rootDir + '/.gitignore');
			const newFile = new vscode.WorkspaceEdit();
			newFile.createFile(filePath, { ignoreIfExists: true });
			const selected = uri.fsPath.replace(`${rootDir}/`, "");
			newFile.insert(filePath, new vscode.Position(0, 0), '\n'+selected);
			vscode.workspace.applyEdit(newFile);
			// Display a message box to the user
			vscode.window.showInformationMessage('Added selected file to .gitignore!');
		}
	});
